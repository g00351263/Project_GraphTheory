...
def compile(pofix):
	...
	for c in pofix:	
		...
		# initial state pined to nfa initial and accept / old accept to  new initial to append right NFA
		elif c == '?':
			nfaAngle = nfastack.pop()
			initial = state()
			accept = state()
			accept = nfaAngle.accept
			initial.edge1 = nfaAngle.initial
			initial.edge2 = nfaAngle.accept
...
infixes = ["a.b.c*", "a.(b|d).c*", "(a.(b|d))*", "a.(b.b)*.c","a*d?c"]
strings = ["", "abc", "abbc", "abcc", "abad", "abbbc","abddddd"]
